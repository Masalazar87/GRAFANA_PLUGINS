...
let estado_uma = leerData("uma","estado")
let alarma_uma = leerData("uma","alarma")
let tret_uma = leerData("uma","tret")
let tsum_uma = leerData("uma","tsum")

function leerData(equipo : any,dato:any){
...
let st_uma: any = [];
let al_uma: any = [];
let temp_ret_uma: any = [];
let temp_sum_uma: any = [];
...
//ESTADOS    
st_uma[i] = estado_uma[(i-1)]?estado_uma[(i-1)]:0
al_uma[i] = alarma_uma[(i-1)]?alarma_uma[(i-1)]:0
if (st_uma[i] === 1) {
    st_uma[i] = st_on;
    msgEstado=" ENCENDIDO"
    imgEquipo=imgUmaAdv;
...
//TEMP. SUMINISTRO Y RETORNO UMAS
    temp_sum_uma[i] = tsum_uma[(i-1)]?tsum_uma[(i-1)]:0
    temp_ret_uma[i] = tret_uma[(i-1)]?tret_uma[(i-1)]:0
    console.log(temp_sum_uma[i],temp_ret_uma[i]) 
    if (temp_sum_uma[i] === null || temp_sum_uma[i] === 0) {
        temp_sum_uma[i] = 0;
        }   else {
            temp_sum_uma[i] = parseFloat(temp_sum_uma[i]).toFixed(1);
        } 
    if (temp_ret_uma[i] === null || temp_ret_uma[i] === 0) {
        temp_ret_uma[i] = 0;
        }   else {
            temp_ret_uma[i] = parseFloat(temp_ret_uma[i]).toFixed(1);
        } 
    }
//--------------------------------------ESTADOS Y ALARMAS DE UPS 200KVA--------------------------------------
let estado_ups = leerData("ups","estado")
let alarma_ups = leerData("ups","alarma")

let st_ups: any = [];
let al_ups: any = [];

    for (let i = 1; i <= 18; i++) {
    if (i<=6){nomEquipo = 'UPS 1-' + (i) + 'A';}
    if (i>6 && i<=12){nomEquipo = 'UPS 2-' + (i-6) + 'A';}
    if (i===13){nomEquipo = 'UPS-CHI-01A';}  
    if (i===16){nomEquipo = 'UPS-CHI-02A';}
    if (i===14){nomEquipo = 'UPS-OFFICES-1'; }
    if (i===15){nomEquipo = 'UPS-SAT-1'; }
    if (i===17){nomEquipo = 'UPS-NOC-2'; }
    if (i===18){nomEquipo = 'UPS-SAT-2A'; }
    let imgUps10kvaAlm= imgPopUp('./ups10_alm.png')
    let imgUps10kvaAdv= imgPopUp('./ups10_adv.png')
    let imgUpsAlm= imgPopUp('./ups_alm.png')
    let imgUpsAdv= imgPopUp('./ups_adv.png')
    let cookieEstado = localStorage.getItem('gyecookie_'+nomEquipo);
    let cookieAlm = localStorage.getItem('gyecookie_'+nomEquipo+'alm');
    //let cookieAdv = localStorage.getItem('gyecookie_'+nomEquipo+'adv');
    st_ups[i] = estado_ups[(i-1)]?estado_ups[(i-1)]:0
    al_ups[i] = alarma_ups[(i-1)]?alarma_ups[(i-1)]:0
//UPS SISTEMA 1&2
    if (st_ups[i] === 1) {
        st_ups[i] = st_on;
        msgEstado=" ENCENDIDO"
        imgEquipo=imgUpsAdv;
        if ((st_ups[i] === st_ups[14])||(st_ups[i] === st_ups[15])){imgEquipo=imgUps10kvaAdv;}
        colorEstado='advertencia'
            PopUp(cookieEstado,nomEquipo,1,nomEquipo)
        }   else {
        msgEstado=" APAGADO"
            imgEquipo=imgUpsAlm;
            if ((st_ups[i] === st_ups[14])||(st_ups[i] === st_ups[15])){imgEquipo=imgUps10kvaAlm;}
                    colorEstado='alarma'
                    st_ups[i] = st_off;
                        PopUp(cookieEstado,nomEquipo,0,nomEquipo)
                }
...
//let ups_chi_vol_in = leerData("tableros","voltaje")
let ups_chi_vol_out = leerData("ups","voltaje")
let ups_chi_cur = leerData("ups","corriente")
/*let VIN_UPS_CHI_1A;
    VIN_UPS_CHI_1A = ups_chi_vol_in[13-1]?ups_chi_vol_in[13-1]:0;*/
let VOUT_UPS_CHI_1A;
    VOUT_UPS_CHI_1A = ups_chi_vol_out[13-1]?ups_chi_vol_out[13-1]:0;
let IOUT_UPS_CHI_1A;
IOUT_UPS_CHI_1A = ups_chi_cur[13-1]?ups_chi_cur[13-1]:0;
...
let estado_chiller = leerData("chiller","estado")
let alarma_chiller = leerData("chiller","alarma")
let temp_ret_chiller = leerData("chiller","tsum")
let temp_sum_chiller = leerData("chiller","tret")

let st_chiller: any = [];
let al_chiller: any = [];
let tsum_chiller: any = [];
let tret_chiller: any = [];
...
for (let i = 1; i <= 4; i++) {
    ...
    st_chiller[i] = estado_chiller[(i-1)]?estado_chiller[(i-1)]:0
    al_chiller[i] = alarma_chiller[(i-1)]?alarma_chiller[(i-1)]:0
...
tsum_chiller[i] = temp_sum_chiller[(i-1)]?temp_sum_chiller[(i-1)]:0;
tret_chiller[i] = temp_ret_chiller[(i-1)]?temp_ret_chiller[(i-1)]:0;

if (temp_sum_chiller[i] === null || temp_sum_chiller[i] === 0) {
    temp_sum_chiller[i] = 0;
    }   else {
        temp_sum_chiller[i] = parseFloat(temp_sum_chiller[i]).toFixed(1);
    } 
if (temp_ret_chiller[i] === null || temp_ret_chiller[i] === 0) {
    temp_ret_chiller[i] = 0;
    }   else {
        temp_ret_chiller[i] = parseFloat(temp_ret_chiller[i]).toFixed(1);
    } 
}
//------------------------------------------------------------------------------------------------------------
...
//BREAKERS TUPS SIS1
let t_ups_in_sis1_vol = leerData("tableros","voltaje")
let t_ups_out_sis1_vol = leerData("tableros","voltaje")
let t_ups_out_sis1_cur = leerData("tableros","corriente")
let t_ups_out_sis1_pot = leerData("tableros","potencia")
let V_TUPSIN_1A_0;
    V_TUPSIN_1A_0 = t_ups_in_sis1_vol[42-1]?t_ups_in_sis1_vol[42-1]:0;
let V_TUPSOUT_1A_0;
    V_TUPSOUT_1A_0 = t_ups_out_sis1_vol[56-1]?t_ups_out_sis1_vol[56-1]:0;
let I_TUPSOUT_1A_0;
    I_TUPSOUT_1A_0 = t_ups_out_sis1_cur[56-1]?t_ups_out_sis1_cur[56-1]:0;
let P_TUPSOUT_1A_0;
    P_TUPSOUT_1A_0 = t_ups_out_sis1_pot[56-1]?t_ups_out_sis1_pot[56-1]:0;

//BREAKERS TUPS SIS2
let t_ups_in_sis2_vol = leerData("tableros","voltaje")
let t_ups_out_sis2_vol = leerData("tableros","voltaje")
let t_ups_out_sis2_cur = leerData("tableros","corriente")
let t_ups_out_sis2_pot = leerData("tableros","potencia")
let V_TUPSIN_2A_0;
    V_TUPSIN_2A_0 = t_ups_in_sis2_vol[49-1]?t_ups_in_sis2_vol[49-1]:0;
let V_TUPSOUT_2A_0;
    V_TUPSOUT_2A_0 = t_ups_out_sis2_vol[64-1]?t_ups_out_sis2_vol[64-1]:0;
let I_TUPSOUT_2A_0;
    I_TUPSOUT_2A_0 = t_ups_out_sis2_cur[64-1]?t_ups_out_sis2_cur[64-1]:0;
let P_TUPSOUT_2A_0;
    P_TUPSOUT_2A_0 = t_ups_out_sis2_pot[64-1]?t_ups_out_sis2_pot[64-1]:0;
//-------------------------------------------------------------------------------------------------------------
...
let t_dp_01a_pot = leerData("tableros","potencia")
let t_dp_02a_pot = leerData("tableros","potencia")
let POT_TDP1A;
let POT_TDP2A;
POT_TDP1A = t_dp_01a_pot[16-1]?t_dp_01a_pot[16-1]:0;
POT_TDP2A = t_dp_02a_pot[29-1]?t_dp_02a_pot[29-1]:0;

let POT_SIS1 = POT_TDP1A; //potencia en TDA-1
let POT_SIS2 = POT_TDP2A; //potencia en TDA-2
...
//-------------------------------------------------------------------------------------------------------------
//---------------------------------- PARAMETROS ESTADOS Y ALARMAS DE PDIS --------------------------------
//
let estado_pdi = leerData("pdu","estado")
let alarma_pdi = leerData("pdu","alarma")
...
for (let i = 1; i <= 20; i++) {
    ...
    let cookieAlm = localStorage.getItem('gyecookie_'+nomEquipo+'alm');
    //let cookieAdv = localStorage.getItem('gyecookie_'+nomEquipo+'adv');
    st_pdi_[i] = estado_pdi[(i-1)]?estado_pdi[(i-1)]:0
    al_pdi_[i] = alarma_pdi[(i-1)]?alarma_pdi[(i-1)]:0
...
principal.ParametrosClima.uma1_TS = temp_sum_uma[1];    principal.ParametrosClima.uma2_TS = temp_sum_uma[2];
principal.ParametrosClima.uma3_TS = temp_sum_uma[3];    principal.ParametrosClima.uma4_TS = temp_sum_uma[4];
principal.ParametrosClima.uma5_TS = temp_sum_uma[5];    principal.ParametrosClima.uma6_TS = temp_sum_uma[6];
principal.ParametrosClima.uma7_TS = temp_sum_uma[7];    principal.ParametrosClima.uma8_TS = temp_sum_uma[8];
principal.ParametrosClima.uma9_TS = temp_sum_uma[9];    principal.ParametrosClima.uma10_TS = temp_sum_uma[10];
principal.ParametrosClima.uma11_TS = temp_sum_uma[11];  principal.ParametrosClima.uma12_TS = temp_sum_uma[12];
//TEMP RETORNO DE UMAS
principal.ParametrosClima.uma1_TR = temp_ret_uma[1];    principal.ParametrosClima.uma2_TR = temp_ret_uma[2];
principal.ParametrosClima.uma3_TR = temp_ret_uma[3];    principal.ParametrosClima.uma4_TR = temp_ret_uma[4];
principal.ParametrosClima.uma5_TR = temp_ret_uma[5];    principal.ParametrosClima.uma6_TR = temp_ret_uma[6];
principal.ParametrosClima.uma7_TR = temp_ret_uma[7];    principal.ParametrosClima.uma8_TR = temp_ret_uma[8];
principal.ParametrosClima.uma9_TR = temp_ret_uma[9];    principal.ParametrosClima.uma10_TR = temp_ret_uma[10];
principal.ParametrosClima.uma11_TR = temp_ret_uma[11];  principal.ParametrosClima.uma12_TR = temp_ret_uma[12];
...
principal.Estados_SIS1.ups1 = st_ups[1];
principal.Estados_SIS1.ups2 = st_ups[2];
principal.Estados_SIS1.ups3 = st_ups[3];  
principal.Estados_SIS1.ups4 = st_ups[4];
principal.Estados_SIS1.ups5 = st_ups[5];  
principal.Estados_SIS1.ups6 = st_ups[6];
principal.Estados_SIS1.upschi1 = st_ups[13];
//ESTADOS UPS 200 KVA SIS2
principal.Estados_SIS2.ups1 = st_ups[7];  
principal.Estados_SIS2.ups2 = st_ups[8];
principal.Estados_SIS2.ups3 = st_ups[9];  
principal.Estados_SIS2.ups4 = st_ups[10];
principal.Estados_SIS2.ups5 = st_ups[11];  
principal.Estados_SIS2.ups6 = st_ups[12];
principal.Estados_SIS2.upschi2 = st_ups[16];
//ALARMAS UPS 200 KVA SIS1
principal.Alarmas_SIS1.ups1 = al_ups[1];
principal.Alarmas_SIS1.ups2 = al_ups[2]; 
principal.Alarmas_SIS1.ups3 = al_ups[3];
principal.Alarmas_SIS1.ups4 = al_ups[4]; 
principal.Alarmas_SIS1.ups5 = al_ups[5]; 
principal.Alarmas_SIS1.ups6 = al_ups[6]; 
principal.Alarmas_SIS1.upschi1 = al_ups[13];
//ALARMAS UPS 200 KVA SIS2
principal.Alarmas_SIS2.ups1 = al_ups[8];
principal.Alarmas_SIS2.ups2 = al_ups[9]; 
principal.Alarmas_SIS2.ups3 = al_ups[10]; 
principal.Alarmas_SIS2.ups4 = al_ups[11]; 
principal.Alarmas_SIS2.ups5 = al_ups[12]; 
principal.Alarmas_SIS2.ups6 = al_ups[13]; 
principal.Alarmas_SIS2.upschi2 = al_ups[16];

//-----------------------------------------ESTADOS DE SISTEMA DE UPS´S-----------------------------------------
principal.Estados_Principales.ups_SIS1 = st_ups[1] === st_on || st_ups[2] === st_on || st_ups[3] === st_on || st_ups[4] === st_on || st_ups[5] === st_on || st_ups[6] === st_on? estadosStyles.on : estadosStyles.sinconexion;
principal.Estados_Principales.ups_SIS2 = st_ups[1] === st_on || st_ups[2] === st_on || st_ups[3] === st_on || st_ups[4] === st_on || st_ups[5] === st_on || st_ups[6] === st_on? estadosStyles.on : estadosStyles.sinconexion;
//-----------------------------------------ALARMAS DE UPS´S 10kva Y RECTIFICADORES-----------------------------------
principal.Alarmas_SIS1.upsoffices_1a = al_ups[14];
principal.Alarmas_SIS1.upssat_1a = al_ups[15];
principal.Alarmas_SIS2.upsnoc_2a = al_ups[17];
principal.Alarmas_SIS2.upssat_2a = al_ups[18];
principal.Estados_SIS1.upsoffices_1a = st_ups[14];
principal.Estados_SIS1.upssat_1a = st_ups[15];
principal.Estados_SIS2.upsnoc_2a = st_ups[17];
principal.Estados_SIS2.upssat_2a = st_ups[18];
...
principal.ParametrosClima_SIS2.T_sumEA1 = tsum_chiller[1];
principal.ParametrosClima_SIS2.T_retEA1 = tret_chiller[1];
principal.ParametrosClima_SIS2.T_sumEA2 = tsum_chiller[2];
principal.ParametrosClima_SIS2.T_retEA2 = tret_chiller[2];
principal.ParametrosClima_SIS1.T_sumEA3 = tsum_chiller[3];
principal.ParametrosClima_SIS1.T_retEA3 = tret_chiller[3];
principal.ParametrosClima_SIS1.T_sumEA4 = tsum_chiller[4];
principal.ParametrosClima_SIS1.T_retEA4 = tret_chiller[4];
//-----------------------------------------BOMBAS Y VALVULAS----------------------------------------------
//--------------------------------------------------------------------------------------------------------
...
principal.ParametrosUPS_SIS1.P_out = Number.parseFloat(P_TUPSOUT_1A_0?.toFixed(2))/10;
...
principal.ParametrosUPS_SIS2.P_out = Number.parseFloat(P_TUPSOUT_2A_0?.toFixed(2))/10;
//----------------------------------------------SENSORES --------------------------------------------------
//---------------------------------------------------------------------------------------------------------
...
// PDI SISTEMA 1
principal.Estados_SIS1.pdu_F1 = st_pdi_[1];
principal.Estados_SIS1.pdu_F2 = st_pdi_[2];
principal.Estados_SIS1.pdu_F3 = st_pdi_[3];
principal.Estados_SIS1.pdu_F4 = st_pdi_[4];
principal.Estados_SIS1.pdu_F5 = st_pdi_[5];
principal.Estados_SIS1.pdu_F6 = st_pdi_[6];
principal.Estados_SIS1.pdu_F7 = st_pdi_[7];
principal.Estados_SIS1.pdu_F8 = st_pdi_[8];
principal.Estados_SIS1.pdu_F9 = st_pdi_[9];
principal.Estados_SIS1.pdu_F10 = st_pdi_[10];
...
principal.Alarmas_SIS1.pdu_F1 = al_pdi_[1];
principal.Alarmas_SIS1.pdu_F2 = al_pdi_[2];
principal.Alarmas_SIS1.pdu_F3 = al_pdi_[3];
principal.Alarmas_SIS1.pdu_F4 = al_pdi_[4];
principal.Alarmas_SIS1.pdu_F5 = al_pdi_[5];
principal.Alarmas_SIS1.pdu_F6 = al_pdi_[6];
principal.Alarmas_SIS1.pdu_F7 = al_pdi_[7];
principal.Alarmas_SIS1.pdu_F8 = al_pdi_[8];
principal.Alarmas_SIS1.pdu_F9 = al_pdi_[9];
principal.Alarmas_SIS1.pdu_F10 = al_pdi_[10];

//PDI SISTEMA 2
principal.Estados_SIS2.pdu_F1 = st_pdi_[11];
principal.Estados_SIS2.pdu_F2 = st_pdi_[12];
principal.Estados_SIS2.pdu_F3 = st_pdi_[13];
principal.Estados_SIS2.pdu_F4 = st_pdi_[14];
principal.Estados_SIS2.pdu_F5 = st_pdi_[15];
principal.Estados_SIS2.pdu_F6 = st_pdi_[16];
principal.Estados_SIS2.pdu_F7 = st_pdi_[17];
principal.Estados_SIS2.pdu_F8 = st_pdi_[18];
principal.Estados_SIS2.pdu_F9 = st_pdi_[19];
principal.Estados_SIS2.pdu_F10 = st_pdi_[20];
...
principal.Alarmas_SIS2.pdu_F1 = al_pdi_[11];
principal.Alarmas_SIS2.pdu_F2 = al_pdi_[12];
principal.Alarmas_SIS2.pdu_F3 = al_pdi_[13];
principal.Alarmas_SIS2.pdu_F4 = al_pdi_[14];
principal.Alarmas_SIS2.pdu_F5 = al_pdi_[15];
principal.Alarmas_SIS2.pdu_F6 = al_pdi_[16];
principal.Alarmas_SIS2.pdu_F7 = al_pdi_[17];
principal.Alarmas_SIS2.pdu_F8 = al_pdi_[18];
principal.Alarmas_SIS2.pdu_F9 = al_pdi_[19];
principal.Alarmas_SIS2.pdu_F10 = al_pdi_[20];
